...
(function(window) {
    ...
    if(!("WebSocket" in window)) {
        alert("Sorry, the build of your browser does not support WebSockets. Please use latest Chrome or Webkit nightly build.");
        return;
    }
    ...
    d3.selectAll(".info-btn").on("click", function() {
        ...
        var drawerContent = d3.select("#drawer-content");
        if (drawerContent.style("width") === "0px") {
            drawerContent
                .transition()
                .style("width", d3.select("#drawer-inner").node().offsetWidth);
        } else {
            drawerContent
                .transition()
                .style("width", "0px");
    ...
    var painter = {
                    ...
                    for (var i = 0; i < cities.length; i++) {
                        var total = linkModels.origins.total(cities[i].counts),
                            latlng = projection([cities[i].longitude,
                                                 cities[i].latitude]),
                            r = Math.ceil(circleScale(total));
                        context.beginPath();
                        context.arc(latlng[0], latlng[1], r, 0, Math.PI * 2);
                        context.fill();
                    }
                }
            },
                ...
                draw: function(context) {
                    for (var i = 0; i < this.data.length; i++) {
                        ...
                        context.arc(this.data[i].source.x, this.data[i].source.y,
                                    this.data[i].source.r || 5, 0, Math.PI * 2);
                        ...
                        context.arc(this.data[i].target.x, this.data[i].target.y,
                                    this.data[i].target.r || 5, 0, Math.PI * 2);
        ...
        _bufferCanvas: {
            canvas: bufCanvas,
            context: bufCanvas.node().getContext("2d")
        },
        ...
        _unbuffer: function() {
            // Draw the buffer image into the canvas
            this._clearContext(this._activeCanvas.context);
            this._activeCanvas.context.drawImage(this._bufferCanvas.canvas.node(), 0, 0);
            this._clearContext(this._bufferCanvas.context);
        },
        ...
        redraw: function() {
            ...
            for (var drawing in this.drawings) {
                drawings.push(this.drawings[drawing]);
            }
            drawings.sort(function (d1, d2) { return d1.order || 0 - d2.order || 0; });

            for (var i = 0; i < drawings.length; i++) {
                if (drawings[i].active) {
                    this._bufferCanvas.context.save()
                    drawings[i].draw(this._bufferCanvas.context);
                    this._bufferCanvas.context.restore();
                }
            }
            this._unbuffer();
        },
        ...
        start: function(interval) {
            var that = this;
            this._interval = setInterval(function() { that.redraw() },
                                         interval || 1000 / 30);
        },
                             ...
                             return [
                                '<div class="bar" style="width: ' +
                                 ...
                                 '<span class="port-circle" style="color:' + colorizer(d.key) +
                                     '">●</span>',
                                 spanWrap(ports[d.key] ? ports[d.key] : "unknown",
					  ["numeric"]),
